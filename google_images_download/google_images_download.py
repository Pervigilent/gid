...
class googleimagesdownload:
	...
	def download_image_thumbnail(self,
		image_url,main_directory,
		dir_name,
		return_image_name,
		print_urls,
		socket_timeout,
	...
	def _get_all_items(self, page, main_directory, dir_name, limit, arguments):
		...
		while count < limit+1:
			...
			if object == "no_links":
				...
				#download the images
				download_status, download_message, return_image_name, absolute_path = self.download_image(object['image_link'],
					object['image_format'],
					main_directory,
					dir_name,
					count,
				...
				if download_status == "success":
					...
					if arguments['thumbnail'] or arguments["thumbnail_only"]:
						if arguments["thumbnail_only"]:
							download_status, download_message_thumbnail, return_image_name = self.download_image_thumbnail(object['image_thumbnail_url'],
								main_directory,
								dir_name,
								return_image_name,
								arguments['print_urls'],
							...
							download_status, download_message_thumbnail, garbage_values = self.download_image_thumbnail(object['image_thumbnail_url'],
								main_directory,
								dir_name,
								return_image_name,
								arguments['print_urls'],
					...
					#print("return_image_name:{0}", return_image_name)
	...
	def download(self, arguments):
		...
		if __name__ != "__main__":
			...
			if 'config_file' in arguments:
				...
				total_errors = 0
				for rec in records:
					paths, errors, items = self.download_executor(rec)
					for i in paths:
						paths_agg[i] = paths[i]
					if not arguments["silent_mode"]:
				...
				return paths_agg, total_errors, items
			# if the calling file contains params directly
			else:
				paths, errors, items = self.download_executor(arguments)
				for i in paths:
					paths_agg[i] = paths[i]
				if not arguments["silent_mode"]:
					if arguments['print_paths']:
						print(paths.encode('raw_unicode_escape').decode('utf-8'))
				return paths_agg, errors, items
	...
	def download_executor(self,arguments):
			...
			for sky in suffix_keywords:			 # 2.for every suffix keywords
				...
				while i < len(search_keyword):	  # 3.for every main keyword
					...
					if arguments['image_directory']:
						...
						# SN: Change this back and send image directory argument through gidsession
						#dir_name = search_term + ('-' + arguments['color'] if arguments['color'] else '')   #sub-directory
						dir_name = search_term.replace(' ', '_') + ('-' + arguments['color'] if arguments['color'] else '')   #sub-directory
		...
		return paths, total_errors, items
